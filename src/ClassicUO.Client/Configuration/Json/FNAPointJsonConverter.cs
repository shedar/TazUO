...
internal sealed class Point2Converter : JsonConverter<Point>
{
    ...
    public override void Write(Utf8JsonWriter writer, Point value, JsonSerializerOptions options) => PointJsonHelper.WritePoint(writer, value.X, value.Y);
}

internal sealed class NullablePoint2Converter : JsonConverter<Point?>
{
    public override Point? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        PointJsonHelper.TryReadPoint(ref reader, out Point point);
